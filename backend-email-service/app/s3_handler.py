# backend-email-service/app/s3_handler.py
import os
import boto3
import uuid
import logging
...
class S3Handler:
    ...
    def upload_file(self, file_data: bytes, filename: str, content_type: str = "application/octet-stream") -> Optional[str]:
        ...
        try:
            ...
            s3_key = f"attachments/{datetime.now().strftime('%Y/%m/%d')}/{unique_filename}"
    ...
    def _extract_s3_key_from_url(self, s3_url: str) -> Optional[str]:
        ...
        try:
            ...
            if f"{self.bucket_name}.s3." in s3_url:
                return s3_url.split(f"{self.bucket_name}.s3.{self.aws_region}.amazonaws.com/")[1]
            return None
        except Exception:
            return None
